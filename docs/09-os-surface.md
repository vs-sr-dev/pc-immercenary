...
| Operamath | −12 | `0x04cca0` | **DivUF16(a, b)** — `(a << 16) / b`, unsigned. The last of the eight to be named, and it has exactly one caller in either image: `ATan2Fine` at `0x04cd00`, which feeds it the smaller of `|dx|`, `|dy|` over the larger, so the answer is the 0 .. 0x10000 its table index wants. See [28](28-what-a-decision-does.md) | one call site |
