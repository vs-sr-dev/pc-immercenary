...
| `0x04cd00` | **OperamathATan2(dx, dy)** — the folio's table-driven arctangent, `0x0590f4`, reached through the divide at `0x04cca0`. `MoverAim` uses this one and `MoverFrame`'s bearing byte uses the octant ramp at `0x0184b4`: two different arctangents a frame | unread |
