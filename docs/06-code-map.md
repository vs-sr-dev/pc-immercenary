...
| `0x0184b4` | **ATan2(dx, dy)** — an octant from the two signs and a compare, then `32 * min / max` inside it. **The shift is a bare 32-bit `lsl #5` in all eight arms and the game lets it overflow**, so a mover more than 1024.0 units from you in its smaller axis gets a bearing byte at `+0x37` that is not a bearing — and that is the byte `MoverDecide`'s sight cone is measured against. `ATan2Fine` is the other one | `tools/armmath.py`, `tools/behave.py` |
| `0x006128` | **MoverShoot(mover)** — `MoverThink`'s third deadline, and the trigger. Refuses without Offense at `+0x5c`, without a target at `+0x70` or in state `0x40`; the range is *half the draw distance + 4 × cid*; the score is 0x40 (0x60 in an encounter or for Silva) plus 16 a unit of arc inside six, plus 50 when the last shot connected, minus 0x40 outside states 6-9, quartered for a named escort; then `RandomBits(8)`, `0x2000` of Offense and `SpawnShot`. `MoverThink` throws the answer away | 464 bytes; `p1e` `0x0198f4`. See [28](28-what-a-decision-does.md) |
| `0x0447fc` | **SpawnShot(mover18)** — a free slot of the sixty-four 92-byte records at `0x08a1ec`: **kind 2**, 2.0 units a tick along `Cos`/`Sin` of `+0x24`, damage `1.0 + maxOffense/16` at `+0x38`, expiry `now + 0x200`, owner at `+0x3c`, and `+0x77` bit 7 on the shooter. Riberto and Loki alone fire off the ground | two callers, `0x006128` and `0x006ac8` |
| `0x00ad78` | **DropMoverShot(mover18)** — clears the low seven bits of the owner's `+0x77` and, for a mover in states 6-9, forces the state back to 0. Called by `CrashMover` and `CreditCrash`, so a crasher loses its hit credit | guards a `0x10101010` sentinel |
| `0x0226f0` | **SetHomeBoxes(flags)** — writes the nine sixteen-byte boxes at `0x060170` from hand-assembled constants, one per lieutenant behind bits 3-11 of the render-flag word. State `0x41` takes the centre of one as its destination; `0x0223ec` reads the same nine to decide whose territory you are in, after a 135-unit test against the DOAsys | not the `PerfectMovers.B3D` rectangle: see [28](28-what-a-decision-does.md) §4 |
| `0x011938` | **GainDOA(who, kind, amount)** — three arms, D, O and both; clamps at the maximum and returns **1 when there was nothing to give**, which is what stops `DrinkFromField` spending a charge on a full drinker. Agility is not in it | `who` of 0 or `0x089d40` is the player |
| `0x01a5ec` | **SpendCharge(col, row)** — takes one off the nine-bit charge of a field cell, floored at zero | eight instructions |
| `0x04cd00` | **ATan2Fine(dx, dy)** — the table-driven arctangent: eight octants out of three sign tests and a compare, then `DivUF16(min, max)` into the 257-word table at `0x0590f4` with the low eight bits interpolating. Right to a unit, where the octant ramp at `0x0184b4` is up to four out. `MoverAim` uses this one and `MoverFrame`'s bearing byte uses the ramp: **two different arctangents a frame** | `tools/armmath.py` |
| `0x04cca0` | **DivUF16(a, b)** — Operamath folio vector **−12**, the one of the eight that had no name. `(a << 16) / b`, unsigned; `ATan2Fine` is its only caller in either image | three instructions |
| `0x006ac8` | **CrowdAim(mover)** — how a Goner that belongs to a crowd aims, and the pack. Quiet, every one of them faces the crowd's own centre at `+4`/`+6`; **alarmed** — bit 8 of the crowd word, which `ResolveHit` sets on whoever's crowd you shot — they all turn on the shooter and **fire on the spot, once an aim**, and go on firing after their Offense reaches zero because the test is `< 0` and the clamp is after the shot | 312 bytes; `MoverAim` arm 0 |
